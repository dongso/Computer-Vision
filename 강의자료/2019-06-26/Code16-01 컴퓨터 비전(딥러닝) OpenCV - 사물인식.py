...
def deepOpenCV():
    ...
    if inImage == None:
        return
    cvPhoto2 = cvPhoto[:]   # pillow 객체가 아닌 openCV 객체를 쓰기 위해서 복사를 해온 것임.
    #################################
    CONF_VALUE = 0.2
    ...
    net = cv2.dnn.readNetFromCaffe("C:/BigData/object-detection-deep-learning/MobileNetSSD_deploy.prototxt.txt",
                                   "C:/BigData/object-detection-deep-learning/MobileNetSSD_deploy.caffemodel")
    image = cvPhoto2
    (h, w) = image.shape[:2]
...
def deep2OpenCV():
    ...
    frameCount = 0
    while True :
        ret, frame = cap.read()  # 현재 한 장면 캡처.
        if not ret:
            break
        frameCount += 1
        if frameCount % 4 ==0 :  # 8은 화면 속도 조절
            ...
            c = cv2.waitKey(1)
            if c == 27: # ESC키 - 반복 하다가 끝내고 싶을 때
                break
            elif c == ord('c') or c == ord('C'):
                captureVideo()
                window.update()
    cap.release()
    cv2.destroyAllWindows() # 열린 창이 닫힘.
...
openCVMenu.add_command(label="사물 인식(딥러닝) - 동작영상", command=deep2OpenCV)
