...
import pymysql
IP_ADDR = '192.168.56.108'; USER_NAME = 'root'; USER_PASS = '1234'
...
def loadCsv(fname):
    ...
    with open(fname, 'r') as rFp:
         for row_list in rFp:
             row, col, value = list(map(int(row_list.split(','))))
...
def saveCsv():
    global window, canvas, paper, filename, inImage, outImage, inH, inW, outH, outW
    saveFp = asksaveasfile(parent=window, mode='wb',
                           defaultextension='*.raw', filetypes=(("CSV 파일", "*.csv"), ("모든 파일", "*.*")))
...
comVisionMenu6.add_command(label="Excel 열기", command=openExcel)
comVisionMenu6.add_command(label="Excel 저장", command=saveExcel)
comVisionMenu6.add_command(label="Excel Art", command=saveExcelArt)
