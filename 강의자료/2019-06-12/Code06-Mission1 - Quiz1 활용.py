...
def moveImage():
    ...
    outImage = malloc(outH, outW)
    ########진짜 컴퓨터 비전 알고리즘 ########
    upDownValue = askinteger("이동할 상하 값을 입력하시오", "상하값~~~>", minvalue =1, maxvalue = 255)
    leftRightValue = askinteger("이동할 좌우 값을 입력하시오", "좌우값~~~>", minvalue =1, maxvalue = 255)
    for i in range(inH):
        for k in range(inW):
            newY = upDownValue + i  # 상하는 행이 이동
            newX = leftRightValue + k # 좌우는 열이 이동
            if 0 <= newX and newX <= 255 and 0 <= newY and newY <= 255:
...
def smallImage():
    global window, canvas, paper, filename, inImage, outImage, inW, inH, outW, outH
    size = askinteger("축소할 배율을 입력하세요.", "축소할 배율 = (2,4,8)", minvalue=2, maxvalue=8)
    outH = int(inH / size)
    outW = int(inW / size)
    ###################################
    outImage = []
    ...
    for i in range(outH):
        for k in range(outW):
            outImage[i][k] = inImage[i * size][k * size] # ex) (4,4) 단계라면, (8,8) 짜리가 (4,4)로 이동함.
    displayImage()

# 확대 -> 배율 (2/4)
def bigImage():
    global window, canvas, paper, filename, inImage, outImage, inW, inH, outW, outH
    size = askinteger("확대할 배율을 입력하세요.", "확대할 배율 = (2,4)", minvalue=2, maxvalue=4)
    outH = int(inH * size)
    outW = int(inW * size)
    ####################################
    outImage = []
    ...
    for i in range(outH):
        for k in range(outW):
            outImage[i][k] = inImage[i // size][k // size] # ex) (4,4) 단계라면, (2,2) 짜리가 (4,4)로 이동함.
    displayImage()
...
comVisionMenu3.add_command(label="오른쪽90도 회전", command=rotateright90)
comVisionMenu3.add_command(label="이동", command=moveImage)
comVisionMenu3.add_command(label="축소", command=smallImage)
comVisionMenu3.add_command(label="확대", command=bigImage)
# comVisionMenu3.add_command(label="(선택) 회전", command=inputAngle)
