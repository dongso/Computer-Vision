...
def openImagePIL() :
    ...
    inImage = Image.open(filename)   # Image에서 import 하기 때문에 open이 가능해짐.
    inW = inImage.width
    inH = inImage.height
...
def displayImagePIL() :
    ...
    window.geometry(str(int(VIEW_Y*1.2)) + 'x' + str(int(VIEW_X*1.2)))  # 벽 -> 화면 꽉 차게 안나오고, 좀 여유있는 공간 두기 위함.
    ...
    for i in numpy.arange(0,outH, step) :
        ...
        rgbStr += '{' + tmpStr + '} '
    paper.put(rgbStr)  # rgbStr을 paper에 넣겠다. 이 코드를 사용하지 않으면 화면에 보여지지 않음.
...
def addImagePIL() :
    ...
    outW = outImage.width    # outW, outH는 색만 변하는 것이기 때문에 필요가 없음.
    outH = outImage.height
    displayImagePIL()
...
def zoominImagePIL() :
    ...
    outW = outImage.width   # outW, outH는 확대시, 틀이 같이 커져야 하기 때문에 필요함.
    outH = outImage.height
    displayImagePIL()
####################
#### 전역변수 선언부 ####
